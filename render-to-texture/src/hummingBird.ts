...
export var birdCamera = engine.RootEntity;
...
export function createHummingBird() {
  ...
  if (birdCamera == engine.RootEntity) {
    let dims = UiCanvasInformation.get(engine.RootEntity);

    const cam = engine.addEntity();
    Transform.create(cam, { parent: bird});
    CameraLayers.create(bird, { layers: [0]})
    TextureCamera.create(cam, {
      width: 512 * 0.4 * dims.width,
      height: 512 * 0.4 * dims.height,
      layer: 1,
      mode: {
        $case: "perspective",
